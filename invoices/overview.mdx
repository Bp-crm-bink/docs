...
| Filter | How to use |
|---|---|
| **Search** | Type an invoice number, contact name, or opportunity name into the search box. Results update as you type. |
| **Status** | Select a status from the **All Statuses** dropdown to show only invoices in that state. |
| **Assignee** | Select a staff member from the **All Assignees** dropdown to show only invoices assigned to that person. |
