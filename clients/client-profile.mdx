...
<Tabs>
  ...
  <Tab title="Household">
    The Household tab lists everyone added as a household member during intake or since. Each member shows their name, relationship, date of birth, sex, US citizen status, and whether they are applying for benefits. You can add new members or edit existing ones from this tab. If your role includes SSN access, you can also add or update a household member's SSN inline — the edit control reads **Add SSN** when none is on file and **Edit SSN** once one exists.
  </Tab>
